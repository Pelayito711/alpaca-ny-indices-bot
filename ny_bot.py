...
def get_current_data(ticker, period="5d", interval="5m"):
...
def main():
    ...
    if os.path.exists(CONFIG_PATH):
        ...
        config = {
            "assets": {
                "QQQ": {"enabled": True, "r_factor": 2.0, "risk_per_trade_pct": 2.0},
                "SPY": {"enabled": False, "r_factor": 2.0, "risk_per_trade_pct": 1.0}
            },
    ...
    if current_time < t_start or current_time > t_end:
        ...
        # Si es después de las 15:55 EST, asegurar cancelación de órdenes pendientes y cierre de posiciones
        if current_time > t_end:
            for symbol in config["assets"].keys():
    ...
    log(f"Ejecución en sesión de Nueva York. Monitoreando activos activos...")

    # Cargar historial de trades del día
    last_trades = {}
    ...
    # Consultar cuenta de Alpaca una sola vez
    try:
    ...
    # Iterar sobre cada activo configurado
    for symbol, asset_conf in config["assets"].items():
        if not asset_conf.get("enabled", False):
            log(f"[{symbol}] Deshabilitado por configuración. Omitiendo.")
            continue
            
        r_factor = asset_conf.get("r_factor", 2.0)
        risk_pct = asset_conf.get("risk_per_trade_pct", 2.0)
        
        # 1. Verificar posición activa en Alpaca
        pos = get_position(symbol)
        if pos is not None:
            log(f"[{symbol}] Posición activa detectada ({pos['qty']} acciones). Bracket Order en control en el servidor.")
            continue
            
        # 2. Verificar si ya hay una orden límite abierta
        open_orders = get_open_orders(symbol)
        if len(open_orders) > 0:
            log(f"[{symbol}] Ya existe {len(open_orders)} orden(es) abierta(s) esperando ejecución. Monitoreando...")
            continue
            
        # 3. Comprobar si ya se operó hoy
        if last_trades.get(symbol) == str(current_date):
                ...
                if risk_pts > 0:
                    ...
                    if qty < 1:
                        if buying_power >= entry_price:
                            qty = 1
                            log(f"[{symbol}] Ajustando a 1 acción completa mínima.")
                        else:
                            log(f"[{symbol}] Saldo insuficiente para comprar 1 acción.")
                            break
                    ...
                    try:
                        ...
                        last_trades[symbol] = str(current_date)
                        with open(LAST_TRADE_PATH, 'w') as f:
                            json.dump(last_trades, f, indent=2)
                        break
                    except Exception as e:
