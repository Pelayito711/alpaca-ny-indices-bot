...
def download_data(ticker, period="30d", interval="5m"):
...
def run_fvg_simulation(df, ticker, r_factor, risk_per_trade):
    t_start = time(9, 30)
...
def main():
    ...
    print("================================================================")
    print("AUTO-OPTIMIZADOR SEMANAL NY INDICES (NASDAQ & S&P 500)")
    print("================================================================\n")
    
    for ticker in list(config["assets"].keys()):
        try:
        ...
        for r in r_factors:
            ret, pf = run_fvg_simulation(df, ticker, r, risk_per_trade=2.0)
            if ret > best_ret:
                best_ret = ret
                best_r = r
                best_pf = pf
        ...
        print(f"-> Retorno 30d: {best_ret:+.2f}% | Profit Factor: {best_pf:.2f}")
        
        # Si el Profit Factor es superior a 1.05, el activo se mantiene habilitado
        if best_pf >= 1.05 and best_ret > 0:
            config["assets"][ticker]["enabled"] = True
            config["assets"][ticker]["r_factor"] = float(best_r)
            print(f"-> Estado: HABILITADO para trading en vivo.")
        else:
            config["assets"][ticker]["enabled"] = False
            config["assets"][ticker]["r_factor"] = float(best_r)
            print(f"-> Estado: DESHABILITADO preventivamente (rendimiento reciente desfavorable).")
            
    with open(CONFIG_PATH, 'w') as f:
        json.dump(config, f, indent=2)
    print(f"\nConfiguración actualizada guardada en: {CONFIG_PATH}")
